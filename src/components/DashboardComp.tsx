...
const DashboardComp = () => {
  ...
  const deleteAutoBookingList = async () => {
    ...
    for (let i = 0; i < booking.length; i++) {
      ...
      if (
        parseInt(booking[i].date.toString()) <
        currentDate - millisecondsInOneDay
      ) {
  ...
  const formatTime = (clock: clockType) => {
    ...
    const formattedMinutes =
      clock.minutes === 0 ? `${clock.minutes}0` : clock.minutes;
  ...
  return (
    <div>
      <div className="mt-10 mx-auto md:mx-24 px-5 flex justify-between">
        <AddButtonComp />
        <button onClick={deleteAutoBookingList}>
          <svg
        ...
        <div className="md:flex space-x-1 md:space-x-4 p-5">
          {["Senin", "Selasa", "Rabu", "Kamis", "Jumat"].map((hari) => {
            const meetingsForDay = booking
              ...
              .sort((a, b) => {
                const dateComparison =
                  parseInt(a.date.toString()) - parseInt(b.date.toString());
            ...
            return (
              <div
                key={hari}
                className="flex flex-col border p-5 rounded-lg mt-3 w-full md:w-56 bg-white shadow-[rgba(13,_38,_76,_0.19)_0px_9px_10px]"
              >
                ...
                <div className="flex flex-col space-y-3 mt-3">
                  {meetingsForDay.length > 0 ? (
                    meetingsForDay.map((book, index) => (
                      <div
                        key={index}
                        className="border p-3 rounded-lg bg-gradient-to-br from-slate-400 to-slate-300 text-slate-950 shadow-[rgba(13,_38,_76,_0.19)_0px_5px_7px]"
                      >
                        <div className="flex justify-end ml-24 scale-75">
                          {/* update */}
                          <button
                            className="mx-3"
                            onClick={() => navigate(`/edit?id=${book._id}`)}
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
                          ...
                          {/* delete */}
                          <button onClick={() => deleteTask(book._id)}>
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
                              viewBox="0 0 24 24"
                        ...
                        <div className="font-bold text-xl">
                          {formatDate(parseInt(book.date.toString()))}{" "}
                        </div>
                        <Separator className="my-3" />
                        <div className="font-bold">
                          {book.event.toUpperCase()}
                        </div>
                        <div>
                          {formatTime(book.clockStart)} -{" "}
                          {formatTime(book.clockEnd)}
                        </div>
                        <div>{book.room}</div>
